...
export const STRINGS_RU: typeof STRINGS_EN = {
  ...
  starfield: {
    ...
    edgeBrightness: 'Яркость в панелях', // Slider dimming every pane against the top strip (English: Brightness in panes)
    edgeBrightnessDesc:
      'Панели приглушены относительно верха, чтобы оставаться позади того, что вы читаете.', // Explains why they are dimmer (English: Panes are dimmed against the top, so they stay behind what you are reading.)
    ...
    graph: 'Граф', // Toggle for the sky behind the graph (English: Graph view)
    graphDesc: 'Позади узлов, и в общем графе, и в локальном.', // Says where it goes (English: Behind the nodes, in both the full graph and the local one.)
